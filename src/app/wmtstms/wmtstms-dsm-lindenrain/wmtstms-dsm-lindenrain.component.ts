...
@Component({
  selector: 'app-wmtstms-dsm-lindenrain',
  templateUrl: './wmtstms-dsm-lindenrain.component.html',
  styleUrls: ['./wmtstms-dsm-lindenrain.component.scss']
})
export class WmtstmsDsmLindenrainComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    const TMS = new TileLayer({
      ...
      source: new XYZ({
        ...
        url: 'https://geoserver.masterarbeit-cog.de/gwc/service/wmts?layer=wmts:dsm_lindenrain&style=&tilematrixset=WebMercatorQuad&Service=WMTS&Request=GetTile&Version=1.0.0&Format=image/vnd.jpeg-png&TileMatrix={z}&TileCol={x}&TileRow={y}'
      }),
    })
    const map = new Map({
      layers: [
